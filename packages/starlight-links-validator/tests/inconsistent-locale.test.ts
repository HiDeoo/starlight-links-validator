...
test('does not build with inconsistent locale links when enabled ', async () => {
  ...
  expectValidationErrors(output, 'en/guides/example.md', [
  ...
  expectValidationErrors(output, 'fr/guides/example.md', [
...
test('does not build with a root locale and inconsistent locale links when enabled ', async () => {
  ...
  expectValidationErrors(output, 'guides/example.md', [['/fr', ValidationErrorType.InconsistentLocale, 10]])
